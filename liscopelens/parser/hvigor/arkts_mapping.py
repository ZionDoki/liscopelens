...
"""
Hvigor ArkTS mapping parser for stage two analysis.
This parser adds file nodes to existing module nodes for all modules with ArkTS-related files.
"""
...
class HvigorArkTSMappingParser(BaseParser):
    """
    Parser for Hvigor project ArkTS file mapping (Stage Two).

    This parser implements stage two of the Hvigor project analysis:
    - Scan ArkTS-related source files in all modules
    ...
    def __init__(self, args: argparse.Namespace, config: Config):
        """
        Initialize the Hvigor ArkTS mapping parser.
        ...
        # File extensions to scan for different file types
        self.arkts_extensions = {".ts", ".ets", ".js"}
        self.resource_extensions = {".json", ".json5", ".xml", ".yaml", ".yml", ".png", ".jpg", ".jpeg", ".svg"}
        self.config_extensions = {".json5", ".ts"}
    ...
    def _get_all_modules(self, context: GraphManager) -> List[Dict[str, Any]]:
        """
        Extract all module nodes from the existing graph context.

        Args:
            context: Graph manager instance

        Returns:
            List of module node data dictionaries
        """
        module_nodes = []
    ...
    def _scan_module_files(self, module_path: Path, module_name: str) -> List[Dict[str, Any]]:
        """
        Scan ArkTS-related files in a module directory.
        ...
        # Define directories to scan for ArkTS modules
        src_dirs = [
            module_path / "src" / "main" / "ets",  # ArkTS source files
        ...
        for src_dir in src_dirs:
            if src_dir.exists() and src_dir.is_dir():
                files.extend(self._scan_directory_recursive(src_dir, module_path, module_name))
    ...
    def _should_include_file(self, file_path: Path) -> bool:
        ...
        skip_dirs = {"build", "node_modules", "oh_modules", ".hvigor", "dist", "out", "target"}
        if any(part in skip_dirs for part in file_path.parts):
            return False

        # Skip C/C++ source files but allow ArkTS files in Native modules
        ...
        suffix = file_path.suffix.lower()
        return (
            suffix in self.arkts_extensions
            or suffix in self.resource_extensions
            or suffix in self.config_extensions
            or file_path.name in {"hvigorfile.ts", "module.json5", "oh-package.json5", "build-profile.json5"}
        )
    ...
    def _extract_file_info(self, file_path: Path, rel_path: Path, module_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if suffix in self.arkts_extensions:
                file_type = "source"
            elif suffix in self.resource_extensions:
                file_type = "resource"
            elif suffix in self.config_extensions or file_path.name.endswith(".json5"):
                file_type = "config"
            else:
                file_type = "other"
    ...
    def parse(self, project_path: str, context: Optional[GraphManager] = None) -> GraphManager:
        """
        Parse the Hvigor project files and add file nodes (Stage Two).

        Args:
            project_path: Path to the Hvigor project root
            context: Existing graph context from stage one

        Returns:
            Updated graph manager with file-level nodes
        """
        if context is None:
            raise ValueError("Stage Two requires existing graph context from Stage One")

        # Stage Two: ArkTS File Mapping
        print("Starting Hvigor project analysis - Stage Two: ArkTS File Mapping")
        ...
        # Get all module nodes from existing context
        module_nodes = self._get_all_modules(context)
        print(f"✓ Found {len(module_nodes)} modules to process")
        ...
        if output := getattr(self.args, "output", None):
            ...
            context.save(output + "/hvigor_arkts_mapping.json")
            print(f"✓ ArkTS mapping saved to: {output}/hvigor_arkts_mapping.json")

        print(f"✓ Stage Two completed. Total files added: {total_files}")
