...
from typing import Dict, Optional, Set, Iterable, List, Union

import json5
from pydantic import ValidationError

from liscopelens.models.hvigor.parser import parse_json_to_dataclass
from liscopelens.models.hvigor import (
    AppProfile,
    ModuleProfile,
    ProjectProfile,
    ModuleBuildProfile,
...
def parse_hvigor_config(json5_path: Path, config_type: str, version: str = "5.0.0") -> Optional[
    Union[
        AppProfile,
        ModuleProfile,
        ProjectProfile,
    ...
    """
    Parse a Hvigor configuration file using the appropriate model.

    Args:
        json5_path: Path to the JSON5 configuration file
        config_type: Type of configuration ("app", "module", "build-profile-project", etc.)
        version: Version of the configuration format

    Returns:
        Parsed configuration model or None if parsing fails
    """
    if not json5_path.exists():
        return None

    try:
        content = read_json5(json5_path)
        if not content:
            return None
        return parse_json_to_dataclass(content, config_type, version)
    except (ValidationError, ValueError) as e:
        warnings.warn(f"Failed to parse {json5_path} as {config_type}: {e}")
        return None
...
class Dependency:
    ...
    @classmethod
    def from_oh_package_deps(cls, deps_dict: Dict[str, str], base_path: Path) -> List["Dependency"]:
        """
        Create Dependency objects from oh-package.json5 dependencies dictionary.

        Args:
            deps_dict: Dependencies dictionary from oh-package.json5
        ...
        for name, version in deps_dict.items():
            ...
            if is_local:
                ...
                local_path = (base_path / relative_path).resolve()

            dependencies.append(cls(name=name, version=version, is_local=is_local, local_path=local_path))
        return dependencies
...
class Module(HvigorEntity):
    ...
    # Parsed configuration models
    pkg_profile_model: Optional[OhPackageModuleProfile] = None
    build_profile_model: Optional[ModuleBuildProfile] = None
    module_profile_model: Optional[ModuleProfile] = None

    # Legacy dict access for backward compatibility
    ...
    def __post_init__(self):
        ...
        if self.build_profile_model and self.build_profile_model.targets:
            for target in self.build_profile_model.targets:
                target_paths = set()
                if target.source:
                    if target.source.abilities:
                        target_paths.update(target.source.abilities)
                    if target.source.sourceRoots:
                        target_paths.update(target.source.sourceRoots)
                    if target.source.pages:
                        target_paths.update(target.source.pages)
                self.targets2path[target.name] = target_paths
        else:
    ...
    def _is_native(self) -> bool:
        ...
        if self.build_profile_model:
            profile_flag = (
                self.build_profile_model.buildOption
                and self.build_profile_model.buildOption.externalNativeOptions is not None
            )
        else:
            # Legacy dict-based check
    ...
    def from_path(cls, root_path: Path, module_path: Optional[Path] = None) -> "Module":
        ...
        module_profile_model = parse_hvigor_config(module_path, "module") if module_path.exists() else None

        # Keep legacy dict access for backward compatibility
        pkg_profile_dict = read_json5(pkg_profile_path)
        build_profile_dict = read_json5(build_profile_path)
        ...
        hm = cls(
            ...
            pkg_profile=pkg_profile_dict,
            build_profile=build_profile_dict,
            module_profile=module_profile_dict,
        )

        # Set module name from module.json5 if available
        if module_profile_model and module_profile_model.module.name:
            hm.name = module_profile_model.module.name
        elif module_profile_dict.get("module", {}).get("name"):
    ...
    def get_module_type(self) -> str:
        """
        Get the module type using model if available, otherwise fall back to dict.

        Returns:
            str: Module type (entry, feature, har, shared)
        """
        if self.module_profile_model and self.module_profile_model.module:
            return self.module_profile_model.module.type
        else:
            return self.module_profile.get("module", {}).get("type", "unknown")

    def get_device_types(self) -> List[str]:
        """
        Get the device types using model if available, otherwise fall back to dict.

        Returns:
            List[str]: List of supported device types
        """
        if self.module_profile_model and self.module_profile_model.module:
            return self.module_profile_model.module.deviceTypes or []
        else:
            return self.module_profile.get("module", {}).get("deviceTypes", [])

    def get_bundle_name(self) -> Optional[str]:
        """
        Get the bundle name from oh-package.json5 using model if available.

        Returns:
            Optional[str]: Bundle name if available
        """
        if self.pkg_profile_model:
            return self.pkg_profile_model.name
        else:
            return self.pkg_profile.get("name")

    def get_license(self) -> Optional[str]:
        """
        Get the license from oh-package.json5 using model if available.

        Returns:
            Optional[str]: License if available
        """
        if self.pkg_profile_model:
            return self.pkg_profile_model.license
        else:
            return self.pkg_profile.get("license")
...
class Project(HvigorEntity):
    ...
    # Parsed configuration models
    build_profile_model: Optional[ProjectProfile] = None
    app_profile_model: Optional[AppProfile] = None
    pkg_profile_model: Optional[OhPackageProjectProfile] = None
    hvigor_profile_model: Optional[HvigorConfig] = None
    ...
    def __post_init__(self):
        ...
        if self.build_profile_model and self.build_profile_model.app.products:
            for product in self.build_profile_model.app.products:
                # Note: targets are not directly in Product model, need to check module targets
                self.products2targets[product.name] = set()
        else:
            # Legacy dict-based parsing
        ...
        # Parse module configurations using model if available
        if self.build_profile_model and self.build_profile_model.modules:
            for module in self.build_profile_model.modules:
                ...
                if module.targets:
                    module_dict["targets"] = [
                        {"name": target.name, "applyToProducts": target.applyToProducts or []}
                        for target in module.targets
                    ]
                self.module_configs[module.name] = module_dict
        else:
            # Legacy dict-based parsing
    ...
    def from_path(cls, root_path: Path) -> "Project":
        ...
        app_profile_model = parse_hvigor_config(app_profile_path, "app")
        pkg_profile_model = parse_hvigor_config(pkg_profile_path, "oh-package-project")

        # Keep legacy dict access for backward compatibility
        hvigor_profile_dict = read_json5(hvigor_profile_path)
        build_profile_dict = read_json5(build_profile_path)
        app_profile_dict = read_json5(app_profile_path)
        pkg_profile_dict = read_json5(pkg_profile_path)
        ...
        tgt_project = cls(
            ...
            pkg_profile_model=pkg_profile_model,
            hvigor_profile=hvigor_profile_dict,
            build_profile=build_profile_dict,
            app_profile=app_profile_dict,
            pkg_profile=pkg_profile_dict,
        )
    ...
    def _discover_modules(self):
        ...
        elif self.build_profile:
            # Legacy dict-based discovery
            modules_config = self.build_profile.get("modules", [])
            for module_config in modules_config:
    ...
    def get_app_bundle_name(self) -> Optional[str]:
        """
        Get the app bundle name using model if available, otherwise fall back to dict.

        Returns:
            Optional[str]: App bundle name if available
        """
        if self.app_profile_model and self.app_profile_model.app:
            return self.app_profile_model.app.bundleName
        else:
            return self.app_profile.get("app", {}).get("bundleName")

    def get_app_version(self) -> Optional[str]:
        """
        Get the app version using model if available, otherwise fall back to dict.

        Returns:
            Optional[str]: App version if available
        """
        if self.app_profile_model and self.app_profile_model.app:
            return self.app_profile_model.app.versionName
        else:
            return self.app_profile.get("app", {}).get("versionName")

    def get_products(self) -> List[str]:
        """
        Get the list of product names using model if available, otherwise fall back to dict.

        Returns:
            List[str]: List of product names
        """
        if self.build_profile_model and self.build_profile_model.app.products:
            return [product.name for product in self.build_profile_model.app.products]
        else:
            products = self.build_profile.get("app", {}).get("products", [])
            return [prod.get("name", "") for prod in products]

    def get_hvigor_version(self) -> Optional[str]:
        """
        Get the Hvigor model version using model if available, otherwise fall back to dict.

        Returns:
            Optional[str]: Hvigor model version if available
        """
        if self.hvigor_profile_model:
            return self.hvigor_profile_model.modelVersion
        else:
            return self.hvigor_profile.get("modelVersion")
